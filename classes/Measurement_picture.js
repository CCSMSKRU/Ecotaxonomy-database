		...
		async.waterfall([
			...
			cb => {
				_t.add_(item, (err, res) => {
					if (err) return cb(new MyError('Не удалось получить measurement_picture', {params: params, err: err}));
					cb(null, res.id);
				});
			},
			// setText:
...
Model.prototype.modifyByList = function (obj, cb) {
    ...
    obj.object = 'measurement_picture';
