...
Model.prototype.sync = function(obj, cb) {
    ...
    let classToClearCache

    const modified_fields = obj.modified_fields

    var dynamic_field = obj.dynamic_field
    var class_profile_
    ...
    async.series({
        ...
        setToRemove: cb => {
            if (obj.to_remove) dynamic_field.to_remove = obj.to_remove
            cb(null)
        },
        // getCP:function(cb){
        //     var o = {
        ...
        getSourceRow: function(cb) {
            if (obj.source_row) {
                dynamic_field.source_row = obj.source_row
                return cb(null)
            }
            var o = {
                command: 'get',
            ...
            _t.api(o, function(err, res) {
                if (err) return cb(new MyError('Не удалось получить dynamic_field.source_class = ' + dynamic_field.source_class, {
                    o: o,
                    err: err
                }))
                if (!res.length) {
            ...
            var o = {
                ...
                params: {
                    param_where: {
                        ...
                        client_object_id: dynamic_field.target_client_object_id
                    },
                    collapseData: false
                }
            }
            _t.api(o, function(err, res) {
                if (err) return cb(new MyError('Не удалось получить client_object_fields_profile для dynamic_field', {
                    o: o,
                    err: err
                }))
            ...
            async.series({
                removeCOP: function(cb) {
                    ...
                    async.eachSeries(Object.keys(dynamic_field.cof_profile_obj_by_name), function(cof_profile_name, cb) {
                        ...
                        _t.api(o, function(err, res) {
                            if (err) return cb(new MyError('Не удалось удалить client_object_fields_profile', {
                                o: o,
                                err: err
                            }))
                            cb(null)
                ...
                async.series({
                    addOrModify: function(cb) {
                        ...
                        if (one_cof.to_add) {
                            ...
                            _t.api(o, function(err, res) {
                                if (err) return cb(new MyError('Не удалось добавить client_object_fields_profile', {
                                    o: o,
                                    err: err
                                }))
                            ...
                            if (modified_fields) {
                                if (cof_profile_name !== dynamic_field.alias + '_real_value') return cb(null)
                                const params = {}
                                Object.keys(o.params).forEach(key => {
                                    if (modified_fields.includes(key)) {
                                        params[key] = o.params[key]
                                    }
                                })
                                if (!Object.keys(params).length) return cb(null)
                                o.params = params
                            }
                            ...
                            o.params.rollback_key = rollback_key
                            _t.api(o, function(err, res) {
                                if (err) return cb(new MyError('Не удалось изменить client_object_fields_profile', {
                                    o: o,
                                    err: err
                                }))
        ...
        syncDynamicKeyword: function(cb) {
            ...
            async.series({
                ...
                modify: function(cb) {
                    if (dynamic_field.to_remove) return cb(null)
                    if (!join_table_keyword) return cb(null)
                    if (join_table_keyword.linked_id === dynamic_field.id_from_source) return cb(null)
                    var o = {
                        command: 'modify',
                        object: 'join_table_keyword',
                        params: {
        ...
        clearCache: function(cb) {
            ...
            if (doNotClearCache) {
                classToClearCache = client_object_profile_.class
                return cb(null)
            }
            var o = {
                command: '_clearCache',
        ...
        if (err) {
            ...
            cb(null, new UserOk('Ок', classToClearCache))
        }
    })
}
