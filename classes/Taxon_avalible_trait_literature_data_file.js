	...
	async.series({
		...
		addPair: function (cb) {
			...
			_t.addPrototype(obj, function(err,res){
				if(err) return cb(new UserError({err: err}));
		...
		//rollback.save({rollback_key:rollback_key, user:_t.user, name:_t.name, name_ru:_t.name_ru || _t.name, method:'uploadDocument', params:obj});
		return cb(null, {id:res.addPair.id, file_id:file_id});
	})
};
