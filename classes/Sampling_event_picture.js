...
Model.prototype.remove_ = function (obj, cb) {
	...
	async.series({
		...
		getPicsCount: cb => {
			if (!sampling_event_id) return cb(null);
...
Model.prototype.addByList = function (obj, cb) {
	...
	async.eachSeries(pictures, function (item, cb) {
		async.waterfall([
			...
			cb => {
				_t.add(item, (err, res) => {
					if (err) return cb(new MyError('Не удалось получить sampling_event_picture', {params: params, err: err}));
					cb(null, res.id);
				});
			},
			// setText:
...
Model.prototype.modifyByList = function (obj, cb) {
	...
	obj.object = 'sampling_event_picture';
