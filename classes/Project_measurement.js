...
Model.prototype.getWithParentalForTCE = function (obj, cb) {
    if (arguments.length == 1) {
        cb = arguments[0];
        obj = {};
    }
    var _t = this;
    var project_id = obj.param_where.project_id;
    ...
    async.series({
                ...
                async.series({
                    getFactors:function(cb){
                        ...
                        _t.get(params, function (err, res) {
                            if (err) return cb(new MyError('Не удалось получить factor',{params : params, err : err})); // Could not get
                            // measurements = measurements.concat(res);
                            measurements = [...measurements, ...res.map(one=>{one.name_with_project = `${one.project}:   ${one.name}`; return one;})];
                            cb(null);
                        });
                    }
                }, cb);
        ...
        cb(null, new UserOk('noToastr',{data: measurements}));
    });
};
