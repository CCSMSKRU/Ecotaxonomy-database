...
Model.prototype.getAll = function (obj, cb) {
    ...
    // получить все по project_id / get everything on project_id
    // Для каждого получить значения в зависимости от типа // For each get values depending on the type
    var project_plot_factor;
    var project_plot_factor_addDATA;
    async.series({
        get:function(cb){
            var params = {
                param_where:{
            ...
            _t.get(params, function(err, res, additionalData){
                if (err) return cb(new MyError('Не удалось получить project_plot_factor',{params:params, err:err})); // Could not get project_plot_factor
                project_plot_factor = res;
                project_plot_factor_addDATA = additionalData;
                cb(null);
            });
        },
        getValues:function(cb){
            if (!project_plot_factor) return cb(null);
            async.eachSeries(project_plot_factor, function(item, cb){
                var o = {
                    command:'get',
                    ...
                    params:{
                        param_where:{
                            ...
                            project_plot_factor_id:item.id
                        },
                        collapseData:false
                    }
                };
                _t.api(o, function (err, res) {
                    if (err) return cb(new MyError('Не удалось plot_factor_value',{o : o, err : err})); // Failed to get project_plot_factor
                    if (res.length > 1) return cb(new MyError('Слишком много пар значений для plot_factor_value', {o:o})); // Failed to get project_plot_factor Too many pairs of values for
                    if (!res.length) return cb(null);
                    item.plot_factor_value_id = res[0].id;
                    cb(null);
                });
            },cb);
        },
        getRealValues:function(cb){
            ...
            if (!project_plot_factor) return cb(null);
            async.eachSeries(project_plot_factor, function(item, cb){
                if (!item.plot_factor_value_id) return cb(null);
                var o = {
                    command:'get',
                    object:item.plot_factor_type_sub_table_name,
                    params:{
                        param_where:{
                            plot_factor_value_id:item.plot_factor_value_id
                        },
                        collapseData:false
                    }
                };
                _t.api(o, function (err, res, additionalData) {
                    if (err) return cb(new MyError('Не удалось get' + item.plot_factor_type_sub_table_name,{o : o, err : err})); // Could not get
                    for (var i in res[0]) {
                        if (['id','plot_factor_value_id'].indexOf(i) !== -1) continue;
                        item[i] = res[0][i];
                    }
                    cb(null);
                });
        ...
        prepareRes:function(cb){
            ...
            for (var i in project_plot_factor) {
                for (var i1 in project_plot_factor[i]) {
                    if (fields.indexOf(i1) === -1) fields.push(i1);
                }

            }
            for (var i2 in project_plot_factor) {
                for (var i3 in fields) {
                    if (typeof project_plot_factor[i2][fields[i3]] === 'undefined') project_plot_factor[i2][fields[i3]] = null;
                }
            }


            var res = funcs.collapseData(project_plot_factor, {
                count: project_plot_factor_addDATA.count,
                count_all: project_plot_factor_addDATA.count_all
            }, project_plot_factor_addDATA.data_columns);
            cb(null, res);
        }
...
Model.prototype.add_ = function (obj, cb) {
    ...
    if (!plot_factor_type_sysname && !obj.plot_factor_type_id) return cb(new UserError('Please, select trait type', {obj:obj}));
                    ...
                    async.series({
                        checkPlotFactor:function(cb){
                            ...
                            _t.getCount(params, function (err, res) {
                                ...
                                if (res.count) return cb(new UserError('Plot factor with this name already exist for this project.',{params:params}));
                                cb(null);
                            });
                        },
                ...
                createTable:function(cb){
                    ...
                    json[alias] = {
                        profile:{
                            ...
                            name_ru:alias,
                            ending:'',
                            server_parent_table:'project_plot_factor',
                            server_parent_key:'project_plot_factor_id'
                        },
                        structure:{
                            id : { type: "bigint", length: "20", notNull: true, autoInc: true, primary_key: true, "quick_search_field":true},
                            project_plot_factor_id: { type: "bigint", length: "20", "quick_search_field":true},
                            project_id : {type: "bigint", length: "20", from_table: "plot_factor_value", keyword: "project_plot_factor_id", return_column: "project_id", is_virtual: true, visible: false, "quick_search_field":true},
...
Model.prototype.setValue = function (obj, cb) {
    ...
    var project_plot_factor, plot_factor_value, plot_factor_type;
    var sub_table_record;
    async.series({
        get:function(cb){
            _t.getById({id:id}, function (err, res) {
                if (err) return cb(new MyError('Не удалось получить project_plot_factor.',{id:id,err:err})); // Could not get
                project_plot_factor = res[0];
                cb(null);
            });
        },
        getOrCreate:function(cb){
            var o = {
                ...
                params:{
                    param_where:{
                        project_plot_factor_id:id,
                        project_id:+obj.project_id || project_plot_factor.project_id
                    },
                    collapseData:false
                }
            };
            _t.api(o, function (err, res) {
                ...
                if (res.length > 1) return cb(new UserError('Слишком много значений для данного свойства и для данного проекта',{o:o, res:res})); // Too many values for this property and for this project
        ...
        getType:function(cb){
            var o = {
                ...
                object:'plot_factor_type',
                params:{
                    id:project_plot_factor.plot_factor_type_id,
                    collapseData:false
                }
            };
            _t.api(o, function (err, res) {
        ...
        getOrCreateSubTableRecord:function(cb){
            ...
            _t.api(o, function (err, res) {
                ...
                if (res.length > 1) return cb(new UserError('Слишком много значений ' + taxon_trait_sub_table_integer + 'для данного taxon_trait_value_id',{o:o, res:res}));  // Too many values
                ...
                if (!sub_table_record) {
                    ...
                    var o = {
                        ...
                        object:plot_factor_type.sub_table_name,
                        params:{
                            taxon_trait_value_id:plot_factor_value.id,
                            rollback_key:rollback_key
                        }
                    };
                    _t.api(o, function (err, res) {
