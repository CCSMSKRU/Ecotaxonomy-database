...
Model.prototype.massiveEditName = function (obj, cb) {
    if (arguments.length === 1) {
        cb = arguments[0];
        obj = {};
    }
    var _t = this;
    if (!obj.name || !obj.extraction_method_ids || obj.extraction_method_ids.length == 0) return cb(new MyError('Не все параметры переданы',{obj:obj})); // Not passed to id
    async.series({
        editName: cb => {
            async.eachSeries(obj.extraction_method_ids, (id, cb) => {
                let o = {
                    id: id,
                    name: obj.name
                }
                _t.modify(o, (err, res) => {
                    if (err) return cb(new MyError('Не удалось изменить имя extraction_method', {obj: o, err: err}))
                    cb(null)
                })
            }, cb)
        }
...
Model.prototype.massiveEditWeblink = function (obj, cb) {
    if (arguments.length === 1) {
        cb = arguments[0];
        obj = {};
    }
    var _t = this;
    if (!obj.weblink || !obj.extraction_method_ids || obj.extraction_method_ids.length == 0) return cb(new MyError('Не все параметры переданы',{obj:obj})); // Not passed to id
    async.series({
        editName: cb => {
            async.eachSeries(obj.extraction_method_ids, (id, cb) => {
                let o = {
                ...
                _t.modify(o, (err, res) => {
                    if (err) return cb(new MyError('Не удалось изменить weblink extraction_method', {obj: o, err: err}))
                    cb(null)
                })
            }, cb)
        }
