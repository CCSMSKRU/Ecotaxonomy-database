(function () {
	...
	var formInstance = MB.Forms.getForm('form_sample_factor', formID);
	...
	var tr = {
		...
		init: function () {
			...
			$(`.hidden_fields`).hide();
			$(`.hidden_fields[data-type='${['INTEGER', 'FLOAT'].indexOf(formInstance.data.data[0].type_sysname) > -1 ? 'NUMBER' : formInstance.data.data[0].type_sysname}']`).show();
			$(`.hidden_fields[data-type='${formInstance.data.data[0].type_sysname}']`).show();
			...
			async.series({
				...
				getValues: cb => {
					...
					if (formInstance.data.data[0].type_sysname !== 'SELECT') return cb(null);

					tr.getValues(function () {
						tr.populateValues();
						cb();
					});
				},
			...
			tr.categoryFilterInit();
		},

		categoryFilterInit: function () {
			const selector = formWrapper.find('.category-filter-holder').find('select');
			const selectParams = {
				class_name: 'sample_factor_category',
			...
			selector.on('select2:select', function (e) {
				tr.populateEvents(tr.filterEvents(tr.events));
			});
			...
			selector.on('select2:opening', function (e) {
				if (selector.data('unselecting')) {
					selector.removeData('unselecting');
					e.preventDefault();
				}
			})
		...
		filterEvents(events) {
			const categoryId = +formWrapper.find('.category-filter-holder').find('select').val();
			const name = formWrapper.find('.category-filter-holder').find('input').val();

			if (categoryId)
				return events.filter(event => (event.category_id === categoryId) && (event.name.includes(name)));
		...
		getLiteratureData: function (cb) {
			var o = {
				command: 'get',
				object: 'sample_factor_literature_data_link',
				params: {
					param_where: {
						sample_factor_id: formInstance.activeId
					}
				}
			};
		...
		populateEvents: function (events) {
			...
			var tpl = `{{#events}}<div class="custom-list-item" data-id="{{id}}">{{name}}<br/><span class="s-event-dates">Type: {{sample_factor_type}}</span></div>{{/events}}`;
		...
		setHandlers: function () {
			...
			formWrapper.find('.custom-list-item').off('click').on('click', function () {
				...
				formInstance.reload(function () {
					formWrapper.find('.name-place').html(formInstance.data.data[0].name); //было .html('Sampling event: ' + formInstance.data.data[0].name);
				});
			});

			formWrapper.find('.load-pictures').off('click').on('click', function () {
				MB.PicEditor.showAddingBox('sample_factor_picture', 'sample_factor_id', id,
			...
			formWrapper.find('.modify-pictures-trait').off('click').on('click', function () {
				MB.PicEditor.showEditor(tr.pictures, 'sample_factor_picture',
					['external_id', 'description'],
					() => {
						tr.reloadPictures();
					});
			...
			formWrapper.find('.tep-litdata, .add-taxon-lit-data, .l-d-watch').off('click').on('click', function () {
				...
				function afterImagesLoaded() {
					let lit_data_obj = new MB.LitData.init(row_id, link_row_id, mo,
						'sample_factor', formInstance.activeId);
			...
			formWrapper.find('.category-filter-holder').find('input').off('input').on('input', function () {
				tr.populateEvents(tr.filterEvents(tr.events));
			})
	...
	let CHTable = {
		...
		setColumnsHandlers: () => {
			...
			$('.is_taxon_checkbox').off('change').on('change', (e) => {
				...
				CHTable.columns[j].is_taxon = $e.attr('checked') === 'checked' ? 'checked' : null;
			});
		},
		setBodyHandlers: () => {
	...
	formWrapper.find('#ch_upload_file').off('click').on('click', () => {
		let o = {
			command: 'checkDataForTableStn',
			object: 'taxon',
			params: {
				properties: CHBox.properties.map(row => {
					return {
	...
	formWrapper.find('#download_file').off('click').on('click', () => {
		...
		socketQuery(o, res => {
			if (res && res.linkName) {
				$("body").prepend(`<a class="temporary_download_link" id="${res.linkName}" href="${res.path + res.filename}" download="${res.filename}"></a>`);
