(function () {
    ...
    tableInstance.ct_instance.ctxMenuData = [
        ...
        },
        {
            ...
            callback: function(){
                ...
                bootbox.dialog({
                    ...
                    '<label style="padding-left: 40px;" class="row-label"><input type="checkbox" id="remove_fields_physical" checked> Удалять физически</label>',
                    buttons:{
                        success: {
        ...
        {
            name: 'option4',
            title: 'Синхронизировать в другой клиентский объект',
            disabled: function(){
                return false;
            },
            callback: function(){
                ...
                var o = {
                    command: 'syncFieldsProfileAnotherCO',
                    object: 'Client_object_profile',
                    params: {
                        client_object_id:co_id
                    }
                ...
                bootbox.dialog({
                    title: 'Синхронизация в другой клиентский объект',
                    message:
                        '<label class="row-label">Синхранизировать в клиентский объект (ID)</label>' +
                        '<input type="text" class="row-label form-control" id="target_client_object_id" value="">' +
                        '<label class="row-label">Синхранизировать поля</label>' +
                        '<input type="text" class="row-label form-control" id="sync_fields_list" value="*">',
                    buttons:{
                        success: {
                            ...
                            callback: function(){
                                var sync_fields_list =          $('#sync_fields_list').val();
                                o.params.sync_fields = sync_fields_list;
                                o.params.target_client_object_id = $('#target_client_object_id').val();
