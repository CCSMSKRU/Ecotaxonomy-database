class File {
    constructor (obj, cb) {
        ...
        this.templates = {
            start_wrapper_file: `
                <div class="wrapper-file"  data-id='{{id}}'>
        ...
        this.observers = []
        if (!this.data.preview) this.previewExtension()
        this.init(cb)
    }
    subscribe(fn) {
        let _t = this;
            ...
            getTypesFile: cb => {
                if (_t.params.select_type_for_new_file) {
                    ...
                    socketQuery(o, res => {
                        ...
                        types_file = Object.values(res.data)
                        cb(null)
                    })
                } else {
                    cb(null)
            ...
            renderBootBox: cb => {
                ...
                if (_t.params.name_for_new_file || _t.params.select_type_for_new_file) {
                    bootbox.dialog({
                        ...
                        message: Mustache.to_html(template, types_file),
                        buttons:{
                            success: {
