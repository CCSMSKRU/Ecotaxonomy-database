...
socket.on('socketQueryCallback', function (callback_id, result, request_time) {
    ...
    if (typeof item.callback === "function") {
        ...
        if (window.debugMode) {
            ...
            //#c60000
            var bg = result.code? (result.code === 10) ? '#c66fbb' : '#c60000' : '#2a711a';
        ...
        if(result.code == 10){
            ...
            var cancelMsg = result.cancelMsg || 'Operation canceled';
            var okBtnText = result.okBtnText || 'Confirm';
            var cancelBtnText = result.cancelBtnText || 'Cancel';
            switch (result.confirmType){
                ...
                default :
                    ...
                    toastr[result.toastr.type](result.toastr.message + '<div style="width: 100%;"><button id="confirm_socket_query_'+btnGuid+'" type="button" class="btn clear">Confirm</button> <button id="cancel_socket_query_'+btnGuid+'" type="button" class="btn clear">Cancel</button></div>','',{
                        "closeButton": false,
                        "debug": false,
                        "newestOnTop": false,
                        "progressBar": false,
