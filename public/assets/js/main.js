/**
 * main.js complex cloud solutions, LLC
 *
 * Mostly unused/depricated functions
 * @type {{}}
 */
...
$('#page-sidebar-toggle').off('click').on('click', function(){
...
/**
 * handlers to close hints and dropdowns
 */
...
$(document).on("click", function (e) {
	...
	if ($('.box_field_image_wrapper').length && !$('.box_field_image_wrapper').is(e.target) && $('.box_field_image_wrapper').has(e.target).length === 0) {
		if ($('.box_field_image_wrapper').attr('remove') === 'remove') {
			$('.box_field_image_wrapper').remove();
		} else {
			$('.box_field_image_wrapper').attr('remove', 'remove')
		}
	}
});
...
(function () {
	...
	MB.Core.switchPage = function (options) {
		...
		if(!options.type || !options.class){
            MB.Core.spinner.stop($('.page-content-wrapper'));
            toastr['warning']('System content error.');
	...
	MB.Core.createButtons = function (instance, selectedRows) {
		...
		var buttonsWrap = (_t.container.find(".lower-buttons-wrapper").last().length > 0)? _t.container.find(".lower-buttons-wrapper").last() : _t.container.find(".upper-buttons-wrapper").last();
		...
		if (buttonsWrap.length && buttons) {
			...
			for (i = 0; i < buttons.length; i++) {
                ...
                if(button.roleAccess){
                    let roleAccessCond = checkRoleAccess(button.roleAccess);
                    if (!roleAccessCond) buttonElem.addClass("hiddenByRole");
                }
	...
	MB.Modal.closefull = function () {
		...
		if ($("#modal_" + MB.Modal.activemodal + "_wrapper .edited").length > 0) {
			return bootbox.dialog({
				message: "Are you sure you want to exit the form without saving the changes?", // Вы уверены что хотите выйти из формы не сохранив изменения?
				title: "There are no saved changes", //Есть не сохраннные изменения
				buttons: {
					success: {
						label: "Yes",
						assName: "green",
	...
	MB.Core.accessForm = function(obj){
        ...
        function runTce(){
            ...
            $('.tce-users-holder').html('');

            $('.tce-users-holder').tce({
                parent_id: obj.id,
                parent_key: 'user_id',
                ...
                get_left : 'user',
                get_right : 'list_of_access',
				right_data_key : 'list_of_access',
                get_right_params: [
                    {
				...
				command_right: 'getUsers',
                lu_command : 'list_of_access',
                add_params: [
                    {
                        key: 'class_operation_id',
                        value: $('.operation-tab.selected').attr('data-id')
                ...
                left_label: 'Availiable users',
                right_label: 'Binded users',
                search_label: 'Search for users',
	    ...
	    let tpl = `<div class="">Access for: <span class="access-object">{{object}}</span></div>
					<div class="">Id: <span class="access-object">{{id}}</span></div>
					<div class="">Name: <span class="access-object">{{name}}</span></div>
					<div class="opeartion-tab-list">{{#operations}}<div class="operation-tab {{#selected}}selected{{/selected}}" data-id="{{id}}" data-name="{{name}}">{{name}}</div>{{/operations}}</div>
					<div class="tce-users-holder"></div>`;
	    let mo = {
	    	object: obj.object,
			id: obj.id,
	    ...
	    socketQuery(o, function(res){
			...
			if(res.class_operation.length == 0){
	    		toastr['warning']('No list access for this object');
				return false;
			}

			for(let i in res.class_operation){
	    		if(res.class_operation[i].name == '*'){
                    res.class_operation[i].selected = true;
                    res.class_operation[i].name = 'All';
				}
			}

			mo.operations = res.class_operation;

            bootbox.dialog({
                title: obj.object + ' access by list configuration',
                ...
                buttons: {
                    success: {
                        label: 'Close',
                        callback: function(){
	                        document.dispatchEvent(new CustomEvent('accessListRefreshed', {
	                        	detail: {
	                        		id: obj.id
	                        	}
	                        }));
            ...
            $('.operation-tab').off('click').on('click', function(){

                $('.operation-tab').removeClass('selected');
...
//	socketParse(res)
	...
	window.replaceAncientSymbols = function (text) {
        let accents = [
        ...
        return function replaceAncientSymbols(text) {
            text = text.replace(/\\/g, '');

            for (const obj of accents) {
                let spec = obj.substr(0, 1);
                let enc = obj.substr(1);

                text = text.replace(new RegExp(enc, 'g'), spec);
            }
...
// flatpickr (Datapicker) локализация
$(function() {
	// Можно закоментить если система на
	if (typeof flatpickr === 'object') {
		flatpickr.localize(flatpickr.l10ns[MB.locale || 'en']);
	}
}());
