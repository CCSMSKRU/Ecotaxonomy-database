...
(function () {
	...
	MB.FormN = function (params) {
        ...
        this.additional_params = params.additional_params || {};
	...
	MB.FormN.prototype.populateFieldByName = function (fieldName) {
		...
		if (_t.activeId !== 'new') {
			...
			field.selValue = (field.profile[0]['lov_columns'] && field.profile[0]['lov_columns'].length > 0)? _t.data.data[0][field.profile[0]['lov_columns'].split(',')[0]] : '';
		} else {
			field.value = '';
			field.selValue = '';
		}
					...
					html =
						`<div data-type="${typeOfEditor}" class="fn-field ${required}" data-column="${field.name}">
							...
							<div class="field_image_wrapper gallery_wrapper ${field.value ? '' : 'empty'}">
								<div class="field_image_not_loaded" style="display: ${img ? 'none' : 'flex'}">Image Not Loaded</div>
...
//                                console.log('Go Go next', toCreateArray[0], toCreateArray[1], currIdx+1);
	...
	MB.FormN.prototype.createChildFrames = function(obj, cb){
		...
		var frm_wrappers = _t.container.find('.fn-child-frm-holder');
							...
							fWrap.each(function(i,e){
								...
								if ((ifWrap.hasClass('fn-readonly-field') || ifWrap.hasClass('form-ro-block')) &&  ifWrap.attr('data-type') != 'checkbox') {
									...
									switch (type) {
                                        ...
                                        //     insert = ifWrap.find('.mce-container')
							...
							if ((fWrap.hasClass('fn-readonly-field') || fWrap.hasClass('form-ro-block')) &&  fWrap.attr('data-type') != 'checkbox') {
								...
								switch (type) {
                                    ...
                                    case 'wysiwyg':
                                        ...
                                        console.log('1712--', fWrap);

                                        let tmce = fWrap.find('.mce-tinymce');

                                        let tmp_id = MB.Core.guid();

                                        tmce.remove();

                                        let field_name = fWrap.attr('data-column');
                                        let field_value = fld.value;

                                        fWrap.find('label').after('<textarea data-id="'+tmp_id+'" rows="10" class="fn-control wysiwyg-wrapper" data-column="' + field_name + '" value="' + field_value + '">' + field_value + '</textarea>');

                                        tinymce.init({
                                            selector: '.wysiwyg-wrapper[data-id="'+tmp_id+'"]',
                                            height: 320,
                                            plugins: "link",
                                            //language: 'ru',
                                            init_instance_callback: function (editor) {
		...
		wrapper.find('.field_image_overlap .load').off('click').on('click', function () {
			...
			var fl = new ImageLoader({
				success: function (file) {
					...
					image.data('smallSrc', file.params.dir + file.name)
					image.data('fullSrc', file.params.dir + file.name)
