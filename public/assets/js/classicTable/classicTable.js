/**
 * classicTable.js - complex cloud solutions, LLC
 *
 * Client-side tables visualization module, works with TableNew.js
 */
...
var classicTable = function () {
    ...
    /**
     * initalization of editors handlers
     * @param colName
     * @param _t
     * @returns {Function}
     */
        ...
        return function (e) {
            if (e.which == 1 && !mouse.isMove && !$(e.target).hasClass('swr') && !$(e.target).parents('.swr').length > 0) {
                ...
                if(_t.changes.length > 0){
                    ...
                    toastr['warning']('You have to save changes before sort.');
                    return false;
...
//					console.log(_t.order_by);
            ...
            if (!$(elem).attr('inited') && !customSelection.isSelection) {
                ...
                $(selInstance).on('changeVal', function (e, was, now) {
                    ...
                    function addChange(selectedRowIndex, selectedColIndex) {
                        ...
                        var col_profile, dataId;
                        for (var index = 0; index < Object.keys(_t.profile.data).length; index++) {
                            if (_t.profile.data[index].column_name == selectedColIndex){
                                col_profile = _t.profile.data[index];
                                break;
                            }
                        }

                        if (col_profile && ['select2withEmptyValue','select2FreeType','select2','select2withAddingNew'].indexOf(col_profile.type_of_editor) !== -1){
                            dataId = _t.data.data[selectedRowIndex][col_profile.lov_return_to_column]
                            if (now.id != dataId) {
                                row.addClass('edited');
                                _t.addChange(chObj);
                            }
    ...
    var initDateTimeHandler = function (elem, additionalClass) {
        var _t = this;
        var $elem = $(elem);
        var is_time = $elem.hasClass('time');
        var is_date = $elem.hasClass('date');
...
//         });
    ...
    /**
     * add instance to global tables array
     * @param item
     */
    ...
    /**
     * remove instance from global tables array
     * @param id
     */
    ...
    /**
     * find table by id in global tables array
     * @param id
     * @returns {*}
     */
    ...
    /**
     * Method will display loading bar after quick search input with custom text, will appears till it calls with false state param
     * @param state
     * @param text
     */

    CTable.prototype.progress_loading = function(state, text){

        let _t = this;
        let tpl = '<div class="ct-progress-loading-holder">'+text+'</div>';

        if(state){
            _t.wrapper.find('.ct-fast-search').after(tpl);
        }else{
            _t.wrapper.find('.ct-progress-loading-holder').remove();
        }
    };


    /**
     * get row data by row index
     * @param index
     * @returns {*}
     */
    ...
    /**
     * get primary keys values by row index
     * @param rowIndex
     * @param newRowId
     * @returns {Array}
     */
    ...
    /**
     * populate editor html by field params
     * @param params
...
//        debugger;
        ...
        //if (!params.editable || ((!params.isNew && params.updatable) || (params.isNew && params.insertable)) ) {
            if (!params.editable && (params.isNew && !params.insertable)) { // next row was here <-
//        || ((!params.isNew && params.insertable) || (params.isNew && params.updatable))
            ...
            if (params.type == 'checkbox') {
                ...
                if (params.name == 'barcode') {
                    ...
                    }else if(params.type == 'colorpicker'){
                        res = '<div class="readonlyCell readonlyColorCell"><div class="readonlyColor" style="background-color: '+params.value+'"></div></div>';
                    }else{
            ...
            switch (params.type) {
                ...
                case 'checkbox':
                    ...
                    break;
                case 'number':
                    res = '<div class="quantity"><input type="number" value="' + params.value + '"></div>';
                    break;
                case 'float2':
                        res = '<div class="quantity"><input type="number" step="0.01" value="' + params.value + '"></div>';
                        break;
                case 'datetime':
                ...
                case 'multiselect':
                case 'select2':
                    res = '<div data-multiselect="' + select2Params.multiSelect + '" data-free_type="' + select2Params.freeType + '" data-with_empty="' + select2Params.withEmptyValue + '" data-val="' + params.selId + '" data-title="' + params.value + '" data-name="' + params.name + '" data-id="' + params.rowId + '" class="ct-select3-wrapper preInit"><span class="select3-inner">' + params.value + '</span><i class="fa fa-angle-down"></i></div>';
                    break;
                case 'colorpicker':
...
//                break;
            ...
            //if (!params.editable || ((!params.isNew && params.updatable) || (params.isNew && params.insertable)) ) {
            if (!params.editable) { // next row was here <-
//        || ((!params.isNew && params.insertable) || (params.isNew && params.updatable))
                ...
                switch (params.type) {
                    ...
                    case 'File':
                        ...
                        break;
	                case 'image':
		                res = `<div class="tbl_field_image_wrapper">${params.value}</div>`;
		                break;
                    case 'select2withEmptyValue':
                    case 'select2FreeType':
                    case 'multiselect':
...
//        }
    ...
    /**
     * remove change from changes array
     * @param change
     */
    ...
    /**
     * get necessary inforamtion for select3 init by column name
     * @param name
     */
    ...
    /**
     * updaes some data from localStorage
     * @param callback
     */
...
//		_t.populateTotalSumms();
    ...
    /**
     * render pagination block using instance profile
     */
    CTable.prototype.renderPaginationBlock = function () {
        var _t = this;
        if (!_t.data['extra_data']) return;
...
// ALSURU
            ...
            _t.wrapper.find('.ct-pagination-pagesCount').html('Pages: ' + _t.pagesCount);
        }
    };

    /**
     * add where params using instance profile
     * @param where
     */
    ...
    /**
     * populate all visual environment (fast search, pagination, functions, buttons etc..)
     * @param callback
     */
    CTable.prototype.renderEnvironment = function (callback) {
        ...
        if (!MB.Tables.getTable(_t.id).parent_id) {
            ...
            '<div class="ct-environment-header">{{tableName}} <span class="ct-total-values-wrapper"><span class="ct-upload-total-summs">Load totals</span></span></div>' +
...
//			console.log('environment rendered');
    ...
    CTable.prototype.render = function (callback) {
        ...
        _t.profile_obj_by_column_name = {};
        for (var v in _t.profile.data) {
            var vItem = _t.profile.data[v];

            _t.profile_obj_by_column_name[vItem.column_name] = vItem;
...
//		console.log(_t);
        ...
        var inlineSaveBtn
                    = (_t.profile['extra_data']['object_profile']['new_command'] || _t.profile['extra_data']['object_profile']['modify_command']) ?
                        (MB.Tables.getTable(_t.id).parent_id == undefined || MB.Tables.getTable(_t.id).parent_id == '') ?
                        '<div class="ct-options-item ct-options-save"><i class="fa fa-save"></i>&nbsp;&nbsp;Save</div>' : '' : '';

        inlineSaveBtn = (MB.Tables.getTable(_t.id).virtual_where == '')? '' : '<div class="ct-options-item ct-options-save"><i class="fa fa-save"></i>&nbsp;&nbsp;Save</div>';

        var fastSearchHtml = (isFastSearch) ? '<div class="ct-fast-search-wrapper">' +
            '<input class="ct-fast-search classicTable-navigate-input" type="text" placeholder="Fast search..."/>' +
            '<div class="ct-loader-percent" id="ct-loader-percent_' + _t.id + '"/>' +
            '</div>' : '';
        ...
        var html = '<div class="classicTableWrap" data-id="' + _t.id + '">' +
            ...
            '<div class="ct-pagination-pagesCount">Pages: ' + _t.pagesCount + '</div>' +
            '</div>' +
            ...
            inlineSaveBtn +
            '<div class="ct-options-item ct-options-filter ' + filterInvisible + '"><i class="fa fa-filter"></i>&nbsp;Filters</div>' +
            ...
            '<ul class="ct-functional-list">' +
                '<li class="exportToExcel exclude-selection"><i class="fa fa-download"></i>&nbsp;Export to excel</li>' +
                '<li class="sendExcelToEmail exclude-selection"><i class="fa fa-envelope-o"></i>&nbsp;Send excel to email</li>' +
                '<li class="ct-exportToFile exclude-selection"><i class="fa fa-download"></i>&nbsp;Export</li>' +
                '<li class="ct-importFromFile exclude-selection"><i class="fa fa-download"></i>&nbsp;Import</li>' +
                '<li class="ct-configureAccess exclude-selection"><i class="fa fa-users"></i>&nbsp;Configure access</li>' +
            '</ul>' +
            ...
            '<div class="tabs-row">' +
            '<div class="tab-btn active" data-id="columns">Columns</div>' +
            '<div class="tab-btn" data-id="options">Options</div>' +
            '</div>' +
        ...
        _t.wrapper.find('.ct-filter div.ct-daterange-wrapper').each(function (index, elem) {
            ...
            var is_time1 = false; // fromInp.hasClass('time');
            var is_date1 = true; // fromInp.hasClass('date');
            var is_datetime1 = (!is_date1 && !is_time1); // Никакой класс не указан - полная дата

            fromInp.flatpickr(
                {
                    dateFormat: (is_time1) ? "H:i:S" : (is_date1) ? "d.m.Y" : "d.m.Y H:i:S",
                    formatDate: (date, format) => {
                        return (is_time1) ? moment(date).format('HH:mm:ss') : (is_date1) ? moment(date).format('DD.MM.YYYY') : moment(date).format('DD.MM.YYYY HH:mm:ss');
                    },
                    parseDate: (date, format) => {
                        return (is_time1) ? moment(date, 'HH:mm:ss').toDate() : (is_date1) ? moment(date, 'DD.MM.YYYY').toDate() : moment(date, 'DD.MM.YYYY HH:mm:ss').toDate();
                    },
                    enableTime: (is_time1 || is_datetime1),
                    time_24hr: true,
                    enableSeconds: true,
                    noCalendar: is_time1,
                    onChange: (selectedDates, dateStr, instance) => {
            ...
            var is_time2 = false; // fromInp.hasClass('time');
            var is_date2 = true; // fromInp.hasClass('date');
            var is_datetime2 = (!is_date2 && !is_time2); // Никакой класс не указан - полная дата

            fromInp.flatpickr(
                {
                    dateFormat: (is_time2) ? "H:i:S" : (is_date2) ? "d.m.Y" : "d.m.Y H:i:S",
                    formatDate: (date, format) => {
                        return (is_time2) ? moment(date).format('HH:mm:ss') : (is_date2) ? moment(date).format('DD.MM.YYYY') : moment(date).format('DD.MM.YYYY HH:mm:ss');
                    },
                    parseDate: (date, format) => {
                        return (is_time2) ? moment(date, 'HH:mm:ss').toDate() : (is_date2) ? moment(date, 'DD.MM.YYYY').toDate() : moment(date, 'DD.MM.YYYY HH:mm:ss').toDate();
                    },
                    enableTime: (is_time2 || is_datetime2),
                    time_24hr: true,
                    enableSeconds: true,
                    noCalendar: is_time2,
                    onChange: (selectedDates, dateStr, instance) => {
        ...
        _t.wrapper.find('.ct-filter div.ct-datetimerange-wrapper').each(function (index, elem) {
            ...
            var is_time1 = false; // fromInp.hasClass('time');
            var is_date1 = false; // fromInp.hasClass('date');
            var is_datetime1 = (!is_date1 && !is_time1); // Никакой класс не указан - полная дата
        ...
        _t.wrapper.find('.ct-filter div.ct-timerange-wrapper').each(function (index, elem) {
            ...
            var is_time1 = true; // fromInp.hasClass('time');
            var is_date1 = false; // fromInp.hasClass('date');
            var is_datetime1 = (!is_date1 && !is_time1); // Никакой класс не указан - полная дата
            ...
            var is_time2 = true; // fromInp.hasClass('time');
            var is_date2 = false; // fromInp.hasClass('date');
            var is_datetime2 = (!is_date2 && !is_time2); // Никакой класс не указан - полная дата
...
//		console.log('handlers setted', new Date(), new Date().getMilliseconds());
        ...
        for (var i in matrix.thead) {
            if (matrix.thead.hasOwnProperty(i)) {
                ...
                if (localStorage && localStorage.columns[columnName]) {
                    ...
                    if (columnWidth) th.width(columnWidth);
    ...
    CTable.prototype.renderTableBody = function (places, matrix, localStorage, convert) {
        ...
        for (var i in matrix.tbody) {
            ...
            var additional_class_fields_profile = _t.data.extra_data.additional_class_fields_profile || {};
            if (matrix.tbody.hasOwnProperty(i)) {
                ...
                if (isAdd) {
                    ...
                    var columns_sorted = []
                    for (var index in _t.profile.data) {
                        columns_sorted.push(_t.profile.data[index].column_name)
                    }

                    // for (var cellName in row) {
                    for (var col_index in columns_sorted) {
                        var cellName = columns_sorted[col_index]

                        var td = $("<td></td>");
                        var profile = _t.profile_obj_by_column_name[cellName];
                        if (!profile) continue;


                        var is_visible = profile['visible'];
                        if (!is_visible) continue;
                        var type = (!convert) ? profile['type_of_editor'] : "text";
                        var isEditable = (!convert) ? profile['editable'] : "FALSE";
                        var isInsertable = profile['insertable'];
                        var isUpdatable = profile['updatable'];
                        var isReqired = profile['required'];
                        var columnName = profile["column_name"];
                        var selId = (row[profile['lov_return_to_column']] != "") ? row[profile['lov_return_to_column']] : row[profile['column_name']];
                        ...
                        var field_add_extra_data = additional_class_fields_profile[cellName];
                        if (field_add_extra_data && field_add_extra_data.is_inherit && (field_add_extra_data.is_inherit.indexOf(row.id) !== -1)){
                            tdW.addClass("is_inherit_field");
                            tdW.attr('title',"Inherited");
                        }
                        td.append(tdW);
    ...
    /**
     * copy table data by cells group, unfinished
     * @param elem
     * @param parent
     */
    ...
    /**
     * render selection using stored info after reload
     */
    ...
    /**
     * get row index by data
     * @param isSelectedIndex
     * @returns {*}
     */
    ...
    CTable.prototype.setHandlers = function () {
        ...
        _t.places.tbody.find('input[type="number"], input[type="float"]').each(function (index, elem) {

            // enhanced input type number

            var holder = $(elem).parents('td');

            if(holder.find('.quantity-button').length > 0 || holder.parents('tr').hasClass('new_row')){
                return;
            }

            $('<div class="quantity-button quantity-up">+</div>').insertAfter(holder.find('.quantity input'));
            $('<div class="quantity-button quantity-down">-</div>').insertBefore(holder.find('.quantity input'));

            holder.find('.quantity').each(function() {
                var spinner = $(this),
            ...
            // END enhanced input type number

        });

	    _t.places.tbody.find('div.tbl_field_image_wrapper').each(function (index, elem) {
		    var body = $('body');
		    var _elem = $(elem);
		    _elem.off('click').on('click', function (e) {
			    var sVal = _elem.html();
			    var colIndex = $(this).parents('td').index();
			    var elem = $(this);
                ...
                let img = sVal ? (sVal.indexOf('upload/') > -1 ? '' : 'upload/') + sVal : '';

			    $('.box_field_image_wrapper').remove();
			    $('body').append(`
							<div class="box_field_image_wrapper field_image_wrapper gallery_wrapper ${sVal ? '' : 'empty'}" style="left: ${left}px; top: ${top}px">
								<div class="field_image_not_loaded">Image Not Loaded</div>
			    ...
			    $('.box_field_image_wrapper .field_image_overlap .load').off('click').on('click', function () {
				    var image = $(this).parents('.field_image_wrapper').find('.field_value');

				    var fl = new ImageLoader({
					    success: function (file) {
						    image.css('background-image', `url('${file.params.dir + file.name}')`);
						    elem.find('.tbl_field_image_wrapper').html(file.name);

						    var chObj = {
							    PRIMARY_KEY_NAMES: _t.primary_keys,
							    PRIMARY_KEY_VALUES: _t.getPKValues(_t.selectedRowIndex, newRowId),
							    CHANGED_COLUMN_NAMES: _t.getColumnNameByIndex(colIndex - 1),
							    CHANGED_COLUMN_VALUES: file.name,
							    ROW: row
						    };
			    ...
			    $('.box_field_image_wrapper .field_image_overlap .remove').off('click').on('click', function () {
				    var image = $(this).parents('.field_image_wrapper').find('.field_value');
				    image.css('background-image', `url('')`);
				    elem.find('.tbl_field_image_wrapper').html('');
        ...
        let fastSearchTimeout = null;
        let fastSearch = (value) => {
            var whereStr = '( ';
            var queryColls = [];
                ...
                for (var k in queryColls) {
                    ...
                    if (dataType == "number" && isBetween && checkMask(value)) {
                        ...
                        fastSearchArray.push({
                            ...
                            group: 'fast_search',
                            comparisonType: 'or',
                            type: _t.profile.extra_data.object_profile.like_type || 'like',
                            val1: value,
                            val2: ''
                        });
...
//			console.log($(this).text());
        ...
        _t.tds.off('click').on('click', function (e) {
            if (!customSelection.isSelection && !customSelection.isSelected) {
                ...
                if (_t.selectedRowIndex != null) {
                    ...
                    let tableN = MB.Tables.getTable(_t.id);
                    let container_to_trigger = (!tableN.parentObject) ? $('.page-content-wrapper') : tableN.parentObject.container;
                    // console.log('-->', container_to_trigger);
                    container_to_trigger.trigger('table_row_click', {e: e, data: _t.selection2});
                }
            }
        });
        ...
        _t.tds.find('input[type="number"], input[type="float"]').off('input').on('input', function () {
        ...
        _t.wrapper.find('.ct-options-drop-filters').off('click').on('click', function () {
            ...
            MB.Tables.getTable(_t.id).fastSearchWhere = [];
            _t.filterWhere = [];
            _t.places.theadRow.find('th').removeClass('asc').removeClass('desc');
            _t.sort = undefined;
        ...
        _t.wrapper.find('.ct-exportToFile').off('click').on('click', function () {
            ...
            if (typeof sel == 'object') {
                return toastr['error']('You must select one row');
            }

            bootbox.dialog({
                message: 'Attention, export  "' + name + '" can take a long time! Continue?',
                title: "Export",
                buttons: {
                    success: {
                        label: "Yes, I'll wait",
                        ...
                        callback: function () {
                            ...
                            var info = toastr.info('The export process is in progress...');
                            ...
                            DOQuery({
                                ...
                                var t2 = new Date();
                                var msg = 'Copy complete. Time: ' + (t2 - t1);
                                console.log(msg);
                                toastr.success(msg);
                                var filename = r.filename;
                                console.log('filename', filename);
                                $("body").prepend('<a style="display:none;" id="need_be_removed" href="/' + filename + '" download>Scheme</a>');
        ...
        _t.wrapper.find('.ct-importFromFile').off('click').on('click', function () {
            var o = {
                ...
                client_object: _t.profile.extra_data.object_profile.name,
                params: {}
            };
            ...
            if(!_t.profile.extra_data.object_profile.import_export_avalible){
                toastr['info']('This table is not allowed for import');
                return false;
            }
            ...
            tpl += '<div class="ct-importFile-wrapper fn-field"><label>Select a file:</label><input type="text" id="ct-importFile-input" class=""/></div>';
            tpl += '</div>';


            bootbox.dialog({
                message: tpl,
                title: "See the fields:",
                buttons: {
                    success: {
                        label: "Fire",
                        className: "",
...
//            }
            ...
            var modalHtml = '<div class="row form-body" id="bootbox' + guid + '">' +
                '<div class="padder5">' +
                '<label class="wid100pr">You must select:' +
            ...
            bootbox.dialog({
                ...
                buttons: {
                    success: {
                        label: "Select file and start importing.",
                        className: "green",
                        callback: function () {
                            var hall_id = selInstance.value.id;
                            if (hall_id < 0) {
                                return toastr.error('Import hall not selected');
                            }
        ...
        _t.wrapper.find('.ct-configureAccess').off('click').on('click', function(){

            if(_t.selectedRowIndex == undefined) {
                toastr['warning']('Please, select a table row to configure access.');
                return false;
            }

            MB.Core.accessForm({
                object: _t.class,
                id: _t.data.data[_t.selectedRowIndex]['id'],
                class_id: _t.profile.extra_data.object_profile.class_id,
                name: _t.data.data[_t.selectedRowIndex]['name']
            });
        ...
        _t.wrapper.find('.exportToExcel').off('click').on('click', function () {
            ...
            var o = {
                ...
                client_object: _t.client_object,//'table_merchant',
                object: _t.class,//'merchant',
                params:{
                    where:[],
                    sort:_t.sort,
                    collapseData:false
            ...
            for(var i in tableNewInstance.fastSearchWhere){
                o.params.where.push(tableNewInstance.fastSearchWhere[i]);
            }
            for(var k in _t.filterWhere){
                o.params.where.push(_t.filterWhere[k]);
            }

            if(_t.profile.extra_data.object_profile.parent_key.length > 0){
                o.params.where.push({
                    key: _t.profile['extra_data']['object_profile']['parent_key'],
                    type: '=',
                    group:'parent',
                    comparisonType: 'and',
                    val1: tableNewInstance.parent_id,
                    val2: ''
                });
            }
            ...
            var modalHtml = '<div class="row form-body" id="bootbox' + guid + '">' +
                '<div class="padder5">' +
                '<label class="wid100pr">Start from:' +
                '<input type="text" class="form-control start_no" value="' + start_no + '"/>' +
                '</label>' +
                '<label class="wid100pr">Limit the number of records:' +
            ...
            bootbox.dialog({
                ...
                buttons: {
                    success: {
                        ...
                        callback: function () {
                            var bootboxContainer = $("#bootbox" + guid);
                            o.params.start_no = +bootboxContainer.find('.start_no').val() || 1;
                            o.params.limit = +bootboxContainer.find('.rows_max_num').val() || 1000;
                            var timeOut = toastr.options.timeOut;
                            var extendedTimeOut = toastr.options.extendedTimeOut;
                            toastr.options.timeOut = 1000000;
                            toastr.options.extendedTimeOut = 100;
                            ...
                            socketQuery(o, function(r){
                                info.fadeOut(600);
                                if (r.code !== 0) {
                                    console.log(r.toastr.message);
                                    return toastr[r.toastr.type](r.toastr.message);
                                ...
                                $("body").prepend('<a style="display:none;" id="' + id + '" href="' + path + filename + '" download></a>');
                                var btn = $('#' + id);
                                btn.on("click", function (e) {
                                    $(this).remove();
                                });
                                btn[0].click();
                                toastr[r.toastr.type](r.toastr.message + '</br><a id="' + id + '"  style="color:blue;" target="_blank"  href="' + path + filename + '">Download</a>'  );
...
//					console.log(MB.Core.validator.time($(this).val()));
        ...
        _t.places.theadRow.find('.swr').on('mousedown', function (e) {
            ...
            _t.tdsToResize = tds;
            _t.wSet = wSet;
            _t.wSet_inner = wSet_inner;
            //console.log(_t.wSet.outerWidth());
            _t.wSetW = _t.wSet.outerWidth();
            _t.wSetW_innerW = _t.wSet_inner.outerWidth();
        ...
        $(document).on('mousemove', function (event) {
            ...
            if (mouse.isDown) {
                if (_t.colInResize) {
                    ...
                    // console.log('margin: ', margin, '_t.wSetW:', _t.wSetW,'_t.columnW: ', _t.columnW );

                    if(_t.columnW <= 50) _t.columnW = 50;


                    for (i in _t.tdsToResize) {
                        td = _t.tdsToResize[i];
                        td.width(_t.columnW + 20);
                    }

                    _t.wSet_inner.width(_t.columnW);
    ...
    /**
     * get filed profile by column name
     * @param name
     * @returns {*}
     */
    ...
    /**
     * hide table in case of all columns hide
     */
    ...
    /**
     * update selected indexes after adding row
     * @param elem
     * @param isAdd
     */
...
//        });
    ...
    /**
     * update editors values by instance data
     */
    ...
    /**
     * return total table summs, unused
     */
...
//				console.log(_t.totalValues);
    ...
    /**
     * define disabled context menu items
     * @param obj
     * @returns {Array}
     */
...
//		console.log(totalResArr);
    ...
    /**
     * get data profile by column name
     * @param column_name
     * @returns {*}
     */
    ...
    /**
     * get local storage instance data
     * @returns {any}
     */
    ...
    /**
     * set instance data to local storage
     * @param data
     */
    ...
    /**
     * open table additional functions menu
     */
    ...
    /**
     * hide table additional functions menu
     */
    ...
    /**
     * create custom buttons, calls from TableNew.js
     */
    CTable.prototype.createCustomButtons = function () {

        var _t = this;

        _t.places.tbody.find('.ct-cell-insert-custom-buttons').each(function (index, elem) {

            var fld_id = $(elem).attr('data-id');

            var tpl = '{{#btns}}<div class="ct-custom-button" data-id="{{id}}"><i class="fa {{icon}}"></i><div class="ct-custom-button-placeholder">{{placeholder}}</div></div>{{/btns}}';
            ...
            if(_t.customButtons){
                ...
                for(var i in _t.customButtons){

                    var btn = _t.customButtons[i];
                    var id = btn.id;

                    if(+id == +fld_id){

                        for(var k in btn.buttons){
                            var kbtn = btn.buttons[k];

                            var btnid = kbtn.id;
                            var icon = kbtn.icon;
                            var placeholder = kbtn.placeholder;
                ...
                $(elem).html(Mustache.to_html(tpl,mo));

            }
        });

        _t.places.tbody.find('.ct-custom-button').off('click').on('click', function () {

            var eid = $(this).attr('data-id');

            var rowidx = $(this).parents('tr').eq(0).index();
            rowidx = rowidx - _t.places.tbody.find('tr.new_row').length;

            var rowdata = _t.data.data[rowidx];

            for(var c in _t.customButtons){
                for(var j in _t.customButtons[c].buttons){

                    if(_t.customButtons[c].buttons[j].id == eid){
                        _t.customButtons[c].buttons[j].callback(rowdata);
                    }
