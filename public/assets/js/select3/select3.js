var select3init = function () {
	...
	var Select3 = function (params) {
        ...
        this.value =                    params.value || {id: "-1", name: "Choose"};
        ...
        this.default_where =            params.default_where || undefined;
...
//        }
	...
	Select3.prototype.init = function () {
		...
		var template = '<div class="select3-wrapper" id="{{id}}">' +
			...
			'<input type="text" class="select3-search2"/>' +
		...
		if (_t.type == 'inited') {
			...
			if (_t.isFilter) {
				o = {
					...
					params: {
						id:_t.rowId,
						parent_id:_t.parent_id,
                        search_keyword: _t.searchKeyword,
                        column_name: _t.column_name,
                        return_id: _t.return_id,
                        return_name: _t.return_name,
                        page_no: _t.page_no
					}
				};
				...
				var row_id = _t.rowId || ((_t.wrapper.parents('.mw-wrap').selector)?
					MB.Forms.getForm(_t.client_object, _t.wrapper.parents('.mw-wrap').attr('id').substr(3)).activeId : null);

                // if(!_t.rowId){
                //     var form_id = MB.Forms.getForm(_t.client_object, _t.wrapper.parents('.mw-wrap').attr('id').substr(3)).activeId;
				// }
				o = {
					...
					params: {
                        ...
                        return_id: _t.return_id,
                        return_name: _t.return_name,
                        default_where: (typeof _t.default_where == 'function')? _t.default_where(_t.wrapper.parents('.fn-field')) : _t.default_where,
                        page_no: _t.page_no
					}
				};
                if(_t.client_object) {
			...
			socketQuery(o, function (res) {
				...
				if (+res.code){
					console.log('В ответ пришла ошибка.',res);
					toastr.error('Something goes wrong, please contact the administrator');
					return;
				}
