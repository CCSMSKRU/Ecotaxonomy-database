...
$(document).ready(function () {
	...
	var show_full_gallery = function show_full_gallery(e) {
		...
		if (log)
			console.log('show_full_gallery all_pics', all_pics);
		...
		if ('smallSrc' in curr_pic_data) {
			for (var i = 0; i < all_pics.length; i++) {
				...
				if (log)
					console.log('show_full_gallery pic', 'smallSrc' in pic_data, 'fullSrc' in pic_data);
	...
	$(document).off('click', '.gallery_image_wrapper').on('click', '.gallery_image_wrapper', function (e) {
		...
		if (!($(e.target).hasClass('remove_image') || $(e.target).closest('.remove_image').length > 0)) {
			show_full_gallery(e);
		}
	});
