...
(function() {
    ...
    $.fn.tce = function (options) {
        var Tce = function (wrapper, obj) {
            ...
            this.search_placeholder = obj.search_placeholder || 'Start to type...';
            ...
            this.left_data = {};
            this.right_data = {};
            this.left_data_key = obj.left_data_key;
            this.right_data_key = obj.right_data_key;
            this.get_left_limit = obj.get_left_limit || 1000;
            this.left_server_search = obj.left_server_search;
            ...
            this.command_left = obj.command_left;
        ...
        Tce.prototype.getData = function () { return new Promise((resolveGetData, reject) => {
            ...
            var lo = {
                command: _t.command_left || "get",
                object: _t.get_left,
                params: {
                    param_where: {},
	                limit: _t.get_left_limit || 100,
                    where: _t.where_left? _t.where_left : undefined
                }
            };
            ...
            socketQuery(lo, async function(res){
                _t.left_data = res[_t.left_data_key || 'data'];
                // console.log(lo, res)
                // debugger
                let ro = {
                ...
                socketQuery(ro, async function (res) {
                    ...
                    // }
                    _t.right_data = res[_t.right_data_key || 'data'];
        ...
        Tce.prototype.populate = function() { return new Promise( (resolveRender, reject) =>{
            ...
            var tpl = "<div id='tce_inner'>" +
                ...
                "<input type='text' class='search_entries fn-control' placeholder='{{search_placeholder}}' value='"+ ((_t.search_val)? _t.search_val : '') +"'/>" +
                "<div class='tce-confirm-search'>Find</div>" +
                "</div>" +
                "</div>" +
                "</div>" +
                "</div>" +
            ...
            for(let i in _t.right_data){
                ...
                let name = getName(right_item, _t.right_name);
            ...
            console.log('right_item.buttons', mO.right_list);
            for (var k in _t.left_data) {
                ...
                if ($.inArray(left_item_id, flat_right_list) == -1) {
                    let name = getName(left_item, _t.left_name);
            ...
            console.log('left_item.buttons', mO.left_list);
        ...
        Tce.prototype.setHandlers = function () {
            ...
            _t.wrapper.find('.tce-confirm-search').off('click').on('click', function(){
                ...
                _t.search_val = val;
                if (_t.left_server_search){
                    _t.where_left = (val)? [{
                        key:_t.where_left_name_key || 'name',
                        type:_t.where_left_type || 'like%',
                        val1:val
                    }] : undefined;
                    _t.reload();
                    return;
...
//        };
    ...
    $.fn.tce_simple = function (options) {
                    ...
                    <div class='row'>
                        <div class="col-md-12">
                            <div class="toBlockCheckboxes">
                                <div class="col-md-12">
                                    <div class="row lh25 tce-search-holder">
                                        <input type="text" class="search_entries fn-control" placeholder="Start to type..." style=" width: 100%; ">
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        TceS.prototype.setHandlers = function () {
	        ...
	        _t.wrapper.find('.search_entries').off('keyup').on('keyup', (e) => {
		        let val = _t.wrapper.find('.search_entries').val();

		        for (let row of _t.data) {
			        if (!val || val.trim().length === 0) {
			            row.visible = true;
                    } else {
			            row.visible = row.name.toLowerCase().indexOf(val.trim().toLowerCase()) > -1;
                    }
		        }
		        // console.log('val', val);
		        // console.log('_t.data', _t.data);
