/**
 * TableNew.js - complex cloud solutions, LLC
 *
 * This client-side class provides work with tables in the system, is responsible for obtaining,
 * displaying and editing data. All tables are available in the browser console in the
 * MB.Tables object.
 */
...
(function () {
	...
	MB.TableN = function (params) {
        ...
        this.pagesCount = 0;
		this.primary_keys = [];
		this.defined_taxon = params.defined_taxon || '';
		...
		this.destroy_on_reload = params.destroy_on_reload; // Существует для таблиц созданных кастомно из кода. Если не передать, то при обновлении формы таблицы будут плодиться. А так уничтожаться
 	};
    ...
    /**
	 * create table and insert it into wrapper
     * @param wrapper
     * @param callback
     */
...
//												console.log(res);
    ...
    /**
	 * Get custom instance script file from public/html/tables/require/table_name.js
     * @param callback
     */
...
//				console.log('asdasd', _t.ct_instance.ctxMenuData);
    ...
    /**
	 * creates custom buttons using object from custom instace script
     * @param callback
     */
	MB.TableN.prototype.createButtons = function (callback) {
        ...
        _t.ct_instance.createCustomButtons();
	};

    /**
	 * render table using client-side visualization module classicTable.js
     * @param callback
     */
    ...
    /**
	 * get table instance data profile from server
     * @param name
     * @param callback
     * @returns {boolean}
     */
	MB.TableN.prototype.getProfile = function (name, callback) {
		...
		if (this.class === '' && this.client_object === '') {
			...
			var profile = localStorage.getItem('tableN_' + _t.coAndClass);
			if (profile !== null){
                _t.doNotUseCache = JSON.parse(profile).extra_data.object_profile.do_not_use_cache_client;
			}
			if (profile !== null && !_t.doNotUseCache) {
				...
				if (!isNaN(+_t.parent_id) && _t.parentObject) {
                    o.params.parent_class___ = _t.parentObject.class;
                    o.params.parent_id___ = +_t.parent_id; //  Это специальные ключи, серверная часть сама вычислит наименование парент ключа / These are special keys, the server part itself will calculate the name of the parent key
                }
				...
				socketQuery(o, function (r) {
					...
					_t.profile = r;
                    _t.doNotUseCache = _t.profile.extra_data.object_profile.do_not_use_cache_client;
    ...
    /**
	 * returns filterWhere oject from classicTable.js instance
     * @returns {Array}
     */
...
//		console.log(_t)
		...
		if(_t.virtual_data != ''){
        	...
        	for(var i in _t.virtual_data){

                o.params[i] = _t.virtual_data[i];
...
//			console.log('oWhere', o);
     ...
     * @param callback
     */
	MB.TableN.prototype.reload = function (params, callback) {
        if (arguments.length == 1) {
            callback = arguments[0];
            params = {};
        }
		...
		var getDataBeforeFunction = (params.fromParentForm && _t.doNotUseCache)? _t.getProfile : function(cb){
		    return cb(null);
		};
		getDataBeforeFunction.call(this, function(){
            _t.getData(function () {
...
//				_t.ct_instance.populateTotalSumms();
    ...
    /**
	 * checkout data changes, using before reloading or pagination to prevent unsaved data loosing
     * @param callback
     * @param actionType
     */
    MB.TableN.prototype.checkForChanges = function (callback, actionType) {
        var _t = this;
        var messages = {
            changePage: '<p>When you go to another page, any unsaved changes will be lost!</p><p>Confirm the transition?</p>',
            reloadTable: '<p>When you reload a table, any unsaved changes will be lost!</p><p>Confirm the reload?</p>'
        };
        var message;
        ...
        if (!localStorage.getItem('neverAskIfChangesOnPaginator'))
            toastrLock({
                ...
                title: 'ATTENTION!',
                buttons: [
                    {name: 'Ok', class: 'danger', type: "SUCCESS", callback: callback},
                    {
                        name: 'Remember selection?', class: 'danger', type: "SUCCESS", callback: function () {
                        localStorage.setItem('neverAskIfChangesOnPaginator', true);
                        callback();
                    }
                    },
                    {name: 'Cancel', class: 'primary', type: "DECLINE", callback: callback}
                ]
            });
        else callback();
    };
    ...
    /**
	 * reinit table rows count by dropdown selection
     */
		...
		var checkForChanges = function (callback, actionType) {
			...
			if (!localStorage.getItem('neverAskIfChangesOnPaginator'))
				toastrLock({
					...
					title: 'ATTENTION!',
					buttons: [
						{name: 'Ok', class: 'danger', callback: callback},
						{
							name: 'Remember selection?', class: 'danger', callback: function () {
							localStorage.setItem('neverAskIfChangesOnPaginator', true);
							callback();
						}
						},
						{name: 'Cancel', class: 'primary'}
					]
				});
			else callback();
		};
...
//					console.log('form update trigger');
		...
		bs.duplicate.off('click').on('click', function () {
			...
			if (rowIndex === null || rowIndex === undefined) {
				toastr['info']('Select the row of the table to duplicate');
				return;
			}
		...
		bs.remove.off('click').on('click', function () {
			...
			if (!removeButton.hasClass("disabled")) {
				...
				if (_t.remove_processing) {
					var diff = moment().diff(_t.remove_processing);
					if (diff < 15000){
						toastr.info('Removing... Expect a result. ' + diff);
						return;
					}
				}
				_t.remove_processing = moment();


				bootbox.dialog({
					title: 'Attention',
					message: 'Are you sure you want to delete these records?', // Вы уверены, что хотите удалить эти записи?
					buttons: {
						success: {
							label: "Confirm",
							className: '',
							callback: function () {
								...
								var pages = Math.ceil( maxRows / rowsPerPage);
								...
								for (var i in primaryKeys) {
									var rowPKs = {
										infoOptions:{
											selector:'#' + _t.wrapper.find('.ct-loader-percent').attr('id')
										}
									};
...
//
				...
				if (o['COMMAND'][0] == 'NEW') {
					...
					if(!res[_t.profile['extra_data']['object_profile']['parent_key']]){
