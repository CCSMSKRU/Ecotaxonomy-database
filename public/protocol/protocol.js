...
var Protocol = function () {
    ...
    this.query = function (obj, callback, do_auth) {
        ...
        var params = JSON.stringify((obj.o && obj.o.params? obj.o.params : {}));
        ...
        $.ajax({
            ...
            data:{
                command: obj.o.command,
                object:obj.o.object,
                params:params
            },
            error: function (err) {
                console.log('Сервер временно недоступен.', err);
                toastr.info('Возможно необходимо добавить Ваш домен в список разрешенных на стороне сервера');
                return callback({code: -1, type: 'error', message: 'Сервер временно недоступен.', err:err});
            },
            success: function (res, textStatus, jqXHR) {
                console.log('RES', res);
                if (res.code === -4 && !do_auth){
                    let o = {
                        command:'login',
                        object:'User',
                        params:{
                    ...
                    self.query({o:o}, function (err, res) {
                        if (err) return callback(err);
                        if (res.code) return callback(res);
                        return self.query(obj, callback);
                        // console.log(textStatus, jqXHR);
                        // toastr.info('Авторизация пройдена, обновите страницу и отправьте запрос снова...')
                        // setTimeout(function(){
                        //
                        // }, 5000)
                    }, true);
    ...
    this.doQuery = function(callback){
        ...
        self.responseContainer.css({backgroundColor:'#f9f9f9'});
        ...
        self.query(data, function (err, res) {
            self.responseContainer.css({backgroundColor:'white'});
            if (loader) loader.stop();
            if (err){
                callback(err);
                err.toastr = err.toastr || {
                    type:'error',
                    message:'Что-то пошло не так, свяжитесь с администратором'
                }
                return toastr[err.toastr.type](err.toastr.message);
